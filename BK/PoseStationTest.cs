...
public class PoseStationTest : MonoBehaviour
{
    ...
    private Numpy.NDarray poses = np.empty(new int[] {0});
    private Numpy.NDarray shapes = np.empty(new int[] {0});
    private Numpy.NDarray transls = np.empty(new int[] {0});
    private float fps = -1;
        ...
        string transl_attribute, string capture_fps_attribute)
    {
        ...
        if(File.Exists(fps_path))
        {
            Numpy.NDarray fps_NDArray = np.load(fps_path);
