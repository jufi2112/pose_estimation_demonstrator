...
public class TcpControlledBody : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        GameObject puppeteer = GameObject.FindWithTag("TCP_Puppeteer");
        if (puppeteer != null)
        {
            m_tcpPuppeteerScript = puppeteer.GetComponent<TcpPuppeteer>();
        }
        ...
        else if(m_poseStationScript is null)
        {
    ...
    void OnDestroy()
    {
        ...
        {
            bool unregisterSuccess = m_poseStationScript.UnregisterBody(gameObject, m_interestedBodyID);
            if (!unregisterSuccess)
                Debug.Log("Could not unregister from the PoseStation");
        }
    }
